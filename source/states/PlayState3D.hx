...
class PlayState3D extends MusicBeatState
{
	public static var instance:PlayState3D = null;
	...
	override public function create()
	{
		...
		canHit = !(ClientPrefs.ghostTapType > 0);
		noMissCount = 0;
		invulnCount = 0;
		...
		healthBarBG = new FlxSprite(0, ClientPrefs.downscroll ? FlxG.height * 0.1 : FlxG.height * 0.875).loadGraphic(Paths.image('healthBar'));
		...
		bgDim.alpha = ClientPrefs.bgDim / 10;
		add(bgDim);
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.song.toLowerCase() == 'fuzzy-logic' && dad.model.fullyLoaded && tv.fullyLoaded)
		{
			...
			if (ClientPrefs.noMouse && !disableMouse)
			{
		...
		if (generatedMusic && PlayState3D.SONG.notes[Std.int(curStep / 16)] != null)
		{
			if (curBeat % 4 == 0)
			{
				// trace(PlayState3D.SONG.notes[Std.int(curStep / 16)].mustHitSection);
			}

			if (camFocus != "dad" && !PlayState3D.SONG.notes[Std.int(curStep / 16)].mustHitSection && autoCam)
			{
				camFocusOpponent();
			}

			if (camFocus != "bf" && PlayState3D.SONG.notes[Std.int(curStep / 16)].mustHitSection && autoCam)
			{
				camFocusBF();
			}
		}
	...
	function updateNote()
	{
		notes.forEachAlive(function(daNote:Note)
		{
			...
			if (ClientPrefs.downscroll)
			{
				daNote.y = (strumLine.y + (getSongPos() - daNote.strumTime) * (0.45 * FlxMath.roundDecimal(PlayState3D.SONG.speed, 2)));
	...
	public function endSong():Void
	{
		...
		// 		// trace(PlayState3D.storyPlaylist[0].toLowerCase() + difficulty);
		...
		// 		PlayState3D.SONG = Song.loadFromJson(PlayState3D.storyPlaylist[0].toLowerCase() + difficulty, PlayState3D.storyPlaylist[0]);
		// 		FlxG.sound.music.stop();

		// 		switchState(new PlayState3D());
	...
	function noteMiss(direction:Int = 1, ?healthLoss:Float = 0.04, ?playAudio:Bool = true, ?skipInvCheck:Bool = false):Void
	{
		...
		{
			health -= healthLoss * ClientPrefs.healthDrainMultiplier;
			if (combo > minCombo)
			{
				gf.playAnim('sad');
				comboUI.breakPopup();
	...
	{
		if (ClientPrefs.ghostTapType > 0 && !canHit)
		{
		}
		else
		{
	...
	function goodNoteHit(note:Note):Void
	{
		...
		else if (!note.wasGoodHit)
		{
			...
			if (note.noteData >= 0)
			{
				health += 0.015 * ClientPrefs.healthMultiplier;
			}
			else
			{
				health += 0.0015 * ClientPrefs.healthMultiplier;
			}
